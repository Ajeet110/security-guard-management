...
// For persistent storage on Render, use: /opt/render/project/persistent/secureguard.db
let dbPath = process.env.DATABASE_URL || path.join(__dirname, 'secureguard.db');

// On Render, check if we should use persistent storage
if (process.env.RENDER && !process.env.DATABASE_URL) {
  // Try to use persistent disk if available
  const persistentPath = '/opt/render/project/persistent/secureguard.db';
  const ephemeralPath = '/opt/render/project/src/server/database/secureguard.db';
  
  console.log(`🔄 Render environment detected`);
  console.log(`📁 Checking for persistent storage at: ${persistentPath}`);
  ...
  try {
    ...
    console.log(`⚠️ Could not use persistent storage: ${error.message}`);
    console.log(`ℹ️ Falling back to: ${ephemeralPath}`);
...
const initDatabase = async () => {
  ...
  try {
    ...
    console.log(`ℹ️ No existing database found: ${err.message}`);
    ...
    if (backups.length > 0) {
      const latestBackup = backups[0]; // Newest first
      console.log(`📦 Found latest backup: ${latestBackup.filename} (${latestBackup.size} bytes)`);
      
      try {
        // Restore from backup
        const restored = restoreBackup(latestBackup.path, dbPath);
        if (restored) {
          console.log(`✅ Database restored from backup: ${latestBackup.filename}`);
          databaseWasRestored = true;
          
          // Reload the restored database
          buffer = fs.readFileSync(dbPath);
          console.log(`✅ Loaded restored database (${buffer.length} bytes)`);
        } else {
          console.log(`❌ Failed to restore from backup, creating new database`);
          buffer = null;
        }
      } catch (restoreError) {
        console.log(`❌ Backup restore failed: ${restoreError.message}`);
        buffer = null;
      }
    } else {
      console.log(`ℹ️ No backups available, will create new database`);
      buffer = null;
    }
  }
